...
void OSCProgram::addMotionTask(const TaskAbstract::SharedPtr &task,
  ...
  motion_tasks_.push_back(TaskBinding(task, BindingAction::ADD, t + duration));
}
...
Vector OSCProgram::getInput() const {
  return qp_solver_->getPrimalSolution().segment(vidx[VAR_CTRL], nu_);
}
...
void OSCProgram::solve(const Real &t, const State &state, const Real &dt) {
  ...
  for (const auto &task : motion_tasks_) {
    Eigen::Ref<Matrix> Hi = conic_data_->H.block(vidx[VAR_QACC], vidx[VAR_QACC],
  ...
  for (const auto &limit : motion_limits_) {
    ...
    auto A = conic_data_->A.block(cidx[CON_LIMITS] + idx_c, vidx[VAR_QACC], m,
                                  vsz[VAR_QACC]);
    // Create bounds
  ...
  conic_data_->A.block(cidx[CON_DYNAMICS], vidx[VAR_QACC], csz[CON_DYNAMICS],
                       vsz[VAR_QACC]) =
      state.getInertiaMatrix().selfadjointView<Eigen::Upper>();
  ...
  for (const auto &a : actuations_) {
    ...
    conic_data_->A.block(cidx[CON_DYNAMICS], vidx[VAR_CTRL], csz[CON_DYNAMICS],
                         vsz[VAR_CTRL]) -= Bi;
  }
  ...
  for (const auto &constraint : constraints_) {
    ...
    Matrix A = Matrix::Zero(m, vsz[VAR_QACC]);
    Vector b = Vector::Zero(m);

    constraint.data->compute(state, A, b);

    // Dynamics
    conic_data_->A.block(cidx[CON_DYNAMICS], vidx[VAR_HOLONOMIC] + idx_v,
                         csz[CON_DYNAMICS], n) -= A.transpose();

    // Holonomic constraint
    conic_data_->A.block(cidx[CON_HOLONOMIC] + idx_c, vidx[VAR_QACC], m,
                         vsz[VAR_QACC]) = A;
    conic_data_->lbA.segment(cidx[CON_HOLONOMIC] + idx_c, m) = -b;
    conic_data_->ubA.segment(cidx[CON_HOLONOMIC] + idx_c, m) = -b;
  ...
  if (size_change) {
    ...
    qp_solver_.reset();
    qp_solver_ = std::make_unique<QPSolver>(num_variables_, num_constraints_);
