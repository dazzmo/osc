...
class OSCProgram {
   public:
    ...
    /**
     * @brief Add a task and the variables the task is associated with
     *
     * @param task
     * @param variables
     * @param duration
     */
    void addTask(const TaskAbstract::SharedPtr &task,
                 const VariableVector &variables, const Real &duration = 0);
    void addContact(const TaskAbstract::SharedPtr &task,
                    const VariableVector &variables, const Real &duration = 0);
    void addLimit(const LimitAbstract::SharedPtr &limit,
                  const Real &duration = 0);
    void addConstraint(const ConstraintAbstract::SharedPtr &constraint,
                       const Real &duration = 0);

    void removeTask(const TaskAbstract::SharedPtr &task,
                    const Real &duration = 0);
    void removeContact(const ContactAbstract::SharedPtr &task,
                       const Real &duration = 0);
    void removeLimit(const LimitAbstract::SharedPtr &limit,
                     const Real &duration = 0);
    void removeConstraint(const ConstraintAbstract::SharedPtr &constraint,
                          const Real &duration = 0);

    // VariableVector addContactVariables(const Size &dim) {}
    // bool removeContactVariables(const Size &dim) {}
    ...
    void remove_motion_task(const string_t &name, const double &t,
        ...
        for (auto it = motion_tasks_.begin(); it != motion_tasks_.end(); ++it) {
            if (it->task->name() == name) {
                ...
                motion_tasks_scheduled_.push_back(
                    {ScheduleTimer(ScheduleTimer::Type::REMOVE, t, duration),
                     *it});
            }
        }
    }
    ...
    /**
     * @brief Removes a contact from the problem, providing a slow release of
     * the contact for program stability.
     *
     * @param name
     * @param t
     * @param duration
    ...
    void remove_contact(const string_t &name, const double &t,
        ...
        for (auto it = contacts_.begin(); it != contacts_.end(); ++it) {
            if (it->contact->name() == name) {
                // If the contact is found, schedule for removal, recording
                // current max normal force allowed for linear scaling
    ...
    /**
     * @brief Computes all tasks and constraints and manages contact handling
     * for the current time step. Use before `qp_data()` to ensure correct
     * problem dimensions are given.
     *
     * @param t
     * @param q
     * @param v
        ...
        for (auto it = schedule.begin(); it != schedule.end();) {
            ...
            if (timer.act(t)) {
                ...
                if (timer.type() == ScheduleTimer::Type::REMOVE) {
                    ...
                    double f = info.f_max * (timer.tf() - t) /
                               (timer.tf() - timer.t0());
            ...
            if (timer.act(t)) {
                ...
                if (timer.type() == ScheduleTimer::Type::ADD) {
                    ...
                    for (auto jt = tasks.begin(); jt != tasks.end();) {
                        VLOG(10)
                            << "Looking to remove task: " << info.task->name();
